...
const BeddingPage = ({ onViewProduct }) => {
    ...
    const headerImage = 'https://plus.unsplash.com/premium_photo-1671269943825-e45b177add8f?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OXx8YmVkfGVufDB8fDB8fHww';
    ...
    useEffect(() => {
        const getProducts = async () => {
            ...
            setError(null);
            try {
                const beddingProducts = await fetchProducts('Bedding', false, 0, sortOrder, filters);
            ...
            } catch (err)
 {
                if (err instanceof TypeError && err.message === 'Failed to fetch') {
                    ...
                    setError(err.message || 'Failed to fetch bedding products.');
                }
                console.error(err);
            } finally {
                setLoading(false);
    ...
    return (
        <>
            <header className="page-header" style={{ backgroundImage: `url(${headerImage})`}}>
                <h1 className="page-header-title">Luxe Bedding</h1>
            </header>
