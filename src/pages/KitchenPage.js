...
const KitchenPage = ({ onViewProduct }) => {
    ...
    const headerImage = 'https://images.unsplash.com/photo-1496116218417-1a781b1c416c?q=80&w=1600&auto=format&fit=crop';
    ...
    useEffect(() => {
        const getProducts = async () => {
            ...
            setError(null);
            try {
                const kitchenProducts = await fetchProducts('Kitchen', false, 0, sortOrder, filters);
                setProducts(kitchenProducts);
            } catch (err) {
                if (err instanceof TypeError && err.message === 'Failed to fetch') {
                    setError('Unable to connect to the server. Please try again later.');
                } else {
                    setError(err.message || 'Failed to fetch kitchen products.');
                }
                console.error(err);
            } finally {
                setLoading(false);
    ...
    return (
        <>
            <header className="page-header" style={{ backgroundImage: `url(${headerImage})`}}>
                <h1 className="page-header-title">Kitchen Essentials</h1>
            </header>
                ...
                <div className="page-content">
                    <div className="page-controls-bar">
                     ...
                     <ProductGrid
                        ...
                        emptyStateOptions={{ title: "No Kitchen Products Found", message: "Try adjusting your filters to find the perfect item for your kitchen." }}
                    />
                </div>
            </div>
        </>
